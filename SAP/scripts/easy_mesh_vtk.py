...
class Easy_Mesh(object):
    ...
    def cropMasterArchBasedOnDie(self, path_file_die, masterArchFileName, output_path='', tol=4):
        """
        ...
        for label in segmentedMeshesDictMaster:
            ...
            segmentedMesh_pc = o3d.geometry.PointCloud(points=o3d.utility.Vector3dVector(np.asarray(segmented_mesh_points)))
        ...
        adjacentLabels = listOfSortedLabels[2:4]

        # get teeth and gingiva labels
        prepLabel = listOfSortedLabels[0]
        gingivaLabel = listOfSortedLabels[1]
        if segmentedMeshesDictMaster[prepLabel].points.shape[0] > segmentedMeshesDictMaster[gingivaLabel].points.shape[0]:
    ...
    def cropAntagonistArchBasedOnDie(self, path_file_die, antagonistArchFileName, output_path='', tol=4.0):
        """
        ...
        self.crop_mesh(teethLabels, gingivaLabel, antagonistArchFileName, 4, output_path=output_path)
...
def GetVTKTransformationMatrix(rotate_X=[-180, 180], rotate_Y=[-180, 180], rotate_Z=[-180, 180],
    ...
    if flags[0] == 0:
        ...
        R_y=np.random.uniform(rotate_Y[0], rotate_Y[1])
        Trans.RotateY(R_y)

    if flags[1] == 0:
        # rotate along Xth axis
        R_x=np.random.uniform(rotate_X[0], rotate_X[1])
        Trans.RotateX(R_x)
    if flags[2] == 0:
        # rotate along Zth axis
        R_z=np.random.uniform(rotate_Z[0], rotate_Z[1])
        Trans.RotateZ(R_z)
    Rotat_M = [0,0,0]

    if flags[3] == 0:
        temp=[np.random.uniform(translate_X[0], translate_X[1]),
    ...
    if flags[4] == 1:
        fix_scale=np.random.uniform(scale_X[0], scale_X[1])
        temp1=[fix_scale,fix_scale,fix_scale]
    ...
    return matrix,Rotat_M,translation,scale_m,flags
